...
### Dev Firmware (unencrypted)
- **`firmware/dev_firmware.bin`** (882 KB) — A "headless" OTA server firmware. Connects to WiFi (reads creds from NVS) and exposes an HTTP OTA endpoint. **Does NOT drive the display** — screen stays off.
...
- Unencrypted `.bin` in `.pio/build/esp32dev/firmware.bin`.
...
If a bad flash bricks the display or the app crashes:

### Quick Recovery (dev firmware via USB)

```bash
./tools/dev init
```

This gets WiFi OTA working again (but display will be off since dev firmware is headless).

### Full Factory Restore (display + BLE)

```bash
./tools/dev init --factory
```

This restores the original manufacturer UI with working display and BLE. Note: the factory firmware does NOT include WiFi OTA, so you'll need to run `./tools/dev init` (without `--factory`) again before deploying apps wirelessly.
...
5. **Dev firmware doesn't drive the display.** After flashing dev firmware, the screen stays off. This is expected — it's a headless OTA server. Flash a community app via WiFi OTA to get the display working.

6. **Factory firmware binaries are encrypted on disk.** They can still be flashed via `esptool.py` because the ESP32 handles encryption transparently. But you cannot inspect their contents or modify them.

7. **The KGFX library.** The krate template includes `kublet/KGFX` — a Kublet-specific graphics helper library. Community apps may or may not use it. The badgers app uses `TFT_eSPI` and `AnimatedGIF` directly.

8. **IP address may change.** The device gets its IP via DHCP. If it changes after a reboot, check your router or use `esp32.local` (mDNS).
